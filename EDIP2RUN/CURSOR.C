...
/*****************************************************************************/
grey_plus() { /*____________________________________________SETS_ONE_FISHK___*/
/*****************************************************************************/
	if( actw == VALW )
	  if( s[acb.csv].ffct & fmask_h[vcb.csv] )
  		if( s[acb.csv].ffct & fmask_l[vcb.csv] ) {                       /* 11 */
      	;
	      }
  	  else {                                                           /* 10 */
      	s[acb.csv].ffct |=  fmask_l[vcb.csv];
        s[acb.csv].fct  |=  (1<<vcb.csv);                   /* correct s.fct */
	      scr[vcb.csv-vcb.up+17][59].ch = 4;
  	    }
	  else
			if( s[acb.csv].ffct & fmask_l[vcb.csv] ) {                       /* 01 */
    		s[acb.csv].ffct &=  ~fmask_l[vcb.csv];
        s[acb.csv].ffct |=   fmask_h[vcb.csv];
	      scr[vcb.csv-vcb.up+17][58].ch = 4;
  	    }
    	else {                                                           /* 00 */
        s[acb.csv].ffct |=   fmask_l[vcb.csv];
        scr[vcb.csv-vcb.up+17][57].ch = 4;
	      }
	}
/*****************************************************************************/
grey_minus() { /*_________________________________________RESETS_ONE_FISHK___*/
/*****************************************************************************/
	if( actw == VALW )
	  if( s[acb.csv].ffct & fmask_h[vcb.csv] )
  		if( s[acb.csv].ffct & fmask_l[vcb.csv] ) {                       /* 11 */
				s[acb.csv].ffct &= ~fmask_l[vcb.csv];
        s[acb.csv].fct  &= ~(1<<vcb.csv);
      	scr[vcb.csv-vcb.up+17][59].ch = 32;
	      }
  	  else {                                                           /* 10 */
				s[acb.csv].ffct &= ~fmask_h[vcb.csv];
      	s[acb.csv].ffct |=  fmask_l[vcb.csv];
	      scr[vcb.csv-vcb.up+17][58].ch = 32;
  	    }
	  else
			if( s[acb.csv].ffct & fmask_l[vcb.csv] ) {                       /* 01 */
    		s[acb.csv].ffct &=  ~fmask_l[vcb.csv];
	      scr[vcb.csv-vcb.up+17][57].ch = 32;
  	    }
    	else {                                                           /* 00 */
    		;
	      }
