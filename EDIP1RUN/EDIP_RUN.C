...
/*****************************************************************************/
	...
	sw_dye_ch( 58,17,58,22,32 ); sw_dye_ch( 62,17,63,22,32 );
...
/*****************************************************************************/
	...
	while( (scr[strn+16][4+j++].ch = getchr(i++)) != 0 );
	scr[strn+16][58].ch = ( s[acb.csv].inf & (1<<valn) ) ? 4 : 0;        /* <> */
	if( s[acb.csv].mrk.inf & (1<<valn) ) {
		scr[strn+16][62].ch = 17;	scr[strn+16][63].ch = 196;               /* <Ä */
		}
...
/*****************************************************************************/
	...
	for( i=vcb.up;i<vcb.up+6 && i<vcb.vln;i++ )
		scr[i-vcb.up+17][58].ch = ( (s[acb.csv].inf & (1<<i)) ? 4 : 0 );   /* <> */
	}
/*****************************************************************************/
...
/*****************************************************************************/
	int i;
	for( i=vcb.up;i<vcb.up+6 && i<vcb.vln;i++ )
		if( s[acb.csv].mrk.inf & (1<<i) ) {
			scr[i-vcb.up+17][62].ch = 17;	scr[i-vcb.up+17][63].ch = 196;     /* <Ä */
			}
		else scr[i-vcb.up+17][62].ch = scr[i-vcb.up+17][63].ch = 32;
	}
/*****************************************************************************/
...
/*****************************************************************************/
	int x = valn - vcb.up + 17;
	if( s[acb.csv].mrk.exp & (1<<valn) )
		if( acb.csv == resatt ){ scr[x][67].ch = 17; scr[x][68].ch = 196;} /* <Ä */
		else                   { scr[x][67].ch = 196;scr[x][68].ch = 16; } /* -> */
	}
/*****************************************************************************/
...
/*****************************************************************************/
	...
	if( s[acb.csv].mrk.hyp & (1<<valn) )
		if( acb.csv == goalatt ){ scr[x][74].ch = 17; scr[x][75].ch = 196;}/* <Ä */
		else                    { scr[x][74].ch = 196;scr[x][75].ch = 16; }/* -> */
	}
/*****************************************************************************/
...
/*****************************************************************************/
	int i,j,k=0;
	for( j=0;j<acb.atn;j++ )
		if( s[j].fct & getint( (long) mofs + 2L * (long)acb.atn *(long)strn + 2L * (long)j ) )
			if( !k ) { k=1; i=j; }
			else return ( 1 );
	if ( k ) s[i].inf &= getint( (long) mofs + 2L * (long)acb.atn * (long)strn + 2L * (long)i );
	else return ( 0 );
	}
/*****************************************************************************/
...
/*****************************************************************************/
	int i,k=0;
	for( i=0;i<acb.atn;i++ ) s[i].inf = s[i].fct;
	thinking( 0 ); contr = 0;
	...
	if( !contr ) {
		for( i=0; i < acb.atn; i++ )
			if( ( s[i].mrk.inf = s[i].fct & ~s[i].inf ) != 0 ) k = 1;
		if( !k ) { thinking_mes( 1 ); return; } /* unsufficient quantity of data */
		for( i=0;i<=acb.atn-1;i++ )	if( s[i].mrk.inf ) { fstinf = i; break; };
		for( i=acb.atn-1;i>=0;i-- )	if( s[i].mrk.inf ) { lstinf = i; break; };
		thinking_mes( 2 );											                      /* success */
		}
	else {
		for( i=0;i<acb.atn;i++ ) { s[i].inf = s[i].fct; s[i].mrk.inf = 0; }
		fstinf = acb.atn - 1; lstinf = 0; thinking_mes( 0 );    /* contradiction */
		}
...
/*****************************************************************************/
	...
	curexp = i;
	for( j=0;j<acb.atn;j++ ) s[j].mrk.exp = getint( mofs + 2L * (long) acb.atn * (long) i + 2L * (long)j );
	if( !contr ) {
...
/*****************************************************************************/
	int i,k;
	if( (k = getint ( mofs + 2L * (long) acb.atn * (long) strn + 2L * (long) goalatt )) == 0 ) return( 0 );
	if(  k & ( 1<<goalval ) ) return( 0 );
	return( 1 );
	}
/*****************************************************************************/
...
/*****************************************************************************/
	...
	curhyp = i;
	for( j=0;j<acb.atn;j++ )
		if( (s[j].mrk.hyp = s[j].fct & getint ( mofs + 2L * (long)acb.atn * (long)i + 2L * (long)j )) != 0 ) k = 1;
	if( !k ) {                                                /* contradiction */
		contr = 1;
    for( i=0;i<acb.atn;i++ ) { s[i].inf = s[i].fct; s[i].mrk.inf = 0; }
...
/*****************************************************************************/
	...
	lseek( hand,6,SEEK_SET );  
	_read( hand,curbase,80 );
  _read( hand,&env.sound,sizeof( short ) );
...
/*****************************************************************************/
  ...
  _write( hand,&env.sound,sizeof( short ) );
...
/*****************************************************************************/
  ...
  for( i=1; i<16; i++ ) {
    ...
    if( i == 12 )        textattr( vers_wc.hc );            /* ªà á­ë¬ EDIP */
	else if ( i < 7 )    textattr( 0x71 );                         /* á¨­¨¬ */
	else        	     textattr( 0x70 );                        /* ç¥à­ë¬ */
